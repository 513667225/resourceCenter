...
public class AliyunOSSUtil {
    ...
    static {
        ENDPOINT = "eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI5NDUxMGZlNC1mZDE3LTQ0MTEtYTVmOS0xOGUxMDM0MDU3ZjMiLCJpYXQiOjE2MDYzNjEyNDAsInN1YiI6Im9zcy1jbi1zaGVuemhlbi5hbGl5dW5jcy5jb20ifQ.rD3PakDG9vzg-DBA2Q_cKocDiMMx0cacUnDm1dY5NNM";
        ACCESS_KEY_ID = "eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI3MWI2YmQ4My0wYzMyLTQ1MDktOTk5Ni1jY2M4NjkzMzlhN2IiLCJpYXQiOjE2MDYzNjEzMTEsInN1YiI6IkxUQUk0RzY2VEQxckRycjhQRW1UaTdEZiJ9.FIc7h2ON3u0ERtMDhFLR_ACwco5zBw82D5kwdUT_qDU";
        ACCESS_KEY_SECERT = "eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI5ZmVlZWY4ZC03NGMyLTRiNjgtOGJhNC0zNmUyYjI2MjVmMDEiLCJpYXQiOjE2MDYzNjEzMzYsInN1YiI6IjFUOEMwY2x6TlAwUDZ4WHQ5b1ZjZUlWUXJCUm1MZiJ9.Myyy7VscP7nLmXFVYjDo23va0Izo_FsOISTYS9aLawA";
        BUCKET_NAME = "eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiIzOTdhZjEzYy01NTZjLTRiMzQtOTg0Ny05NTQ4MzQ5OWJjNmQiLCJpYXQiOjE2MDYzNjEzNjMsInN1YiI6Inl1YW4tcGluLWh1aSJ9.PhAaHTNXu2DnKsDocz01cykOWdgI4kHgTsvKklxM2W0";
        FOLDER_NAME = "";
    }


    public static R deleteImage(String imageName) throws Exception{
        OSSClient client = new OSSClient(JwtUtil.parseJWT(ENDPOINT).getSubject(),
                JwtUtil.parseJWT(ACCESS_KEY_ID).getSubject(),JwtUtil.parseJWT(ACCESS_KEY_SECERT).getSubject());
        client.deleteObject(JwtUtil.parseJWT(BUCKET_NAME).getSubject(), imageName);
        client.shutdown();
        return R.success();
    }
    ...
    public static OSS getOSSClient() throws Exception{
        ...
        return new OSSClientBuilder().build(JwtUtil.parseJWT(ENDPOINT).getSubject(),JwtUtil.parseJWT(ACCESS_KEY_ID).getSubject(),JwtUtil.parseJWT(ACCESS_KEY_SECERT).getSubject());
    }
...
//            metadata.setContentDisposition("filename/filesize=" + fileName + "/" + fileSize + "Byte.");
    ...
    public static String getUrl(String key) throws Exception{
        ...
        // 生成URL
        URL url = ossClient.generatePresignedUrl(JwtUtil.parseJWT(AliyunOSSUtil.BUCKET_NAME).getSubject(), key, expiration);
    ...
    public static String uploadVideo(MultipartFile file,Integer shopId,String paper) throws Exception {
        ...
        OSSClient ossClient = (OSSClient) AliyunOSSUtil.getOSSClient();
        uploadVideo2OSS(ossClient, input, JwtUtil.parseJWT(AliyunOSSUtil.BUCKET_NAME).getSubject(), floder, beforeFileName, afterFileName);
...
//        BufferedImage bufImg = ImageIO.read(input);
        ...
        OSSClient ossClient = (OSSClient) AliyunOSSUtil.getOSSClient();
        uploadImg2OSS(ossClient, bufImg, JwtUtil.parseJWT(AliyunOSSUtil.BUCKET_NAME).getSubject(), floder, beforeFileName, afterFileName);
        return getUrl(floder+afterFileName);
    }
